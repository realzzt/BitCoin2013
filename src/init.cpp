...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in %s/kB) smaller than this are considered zero fee for relaying, mining and transaction creation (default: %s)"), CURRENCY_UNIT, FormatMoney(DEFAULT_MIN_RELAY_TX_FEE)));
	strUsage += HelpMessageOpt("-maxtxfee=<amt>", strprintf(_("Maximum total fees (in %s) to use in a single wallet transaction or raw transaction; setting this too low may abort large transactions (default: %s)"), CURRENCY_UNIT, FormatMoney(DEFAULT_TRANSACTION_MAXFEE)));
...
std::string LicenseInfo()
{
	...
	return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 2009, COPYRIGHT_YEAR) + " ") + "\n" +
		...
		strprintf(_("Please contribute if you find %s useful. "
			"Visit %s for further information about the software."), PACKAGE_NAME, URL_WEBSITE) +
		"\n" +
...
bool AppInitServers(boost::thread_group &threadGroup)
{
	...
	if(GetBoolArg("-rest", DEFAULT_REST_ENABLE) &&
		!StartREST())
	{
		return false;
	}
...
void InitParameterInteraction()
{
	...
	if(GetBoolArg("-blocksonly", DEFAULT_BLOCKSONLY))
	{
		if(SoftSetBoolArg("-whitelistrelay", false))
			LogPrintf("%s: parameter interaction: -blocksonly=1 -> setting -whitelistrelay=0\n", __FUNCTION__);	// walletbroadcast is disabled in CWallet::ParameterInteraction()
	}
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	if(nMempoolSizeMax < 0 ||
		nMempoolSizeMax < nMempoolSizeMin)
	{
	...
	if((!fEnableReplacement) &&
		mapArgs.count("-mempoolreplacement"))
	{
	...
	if(strSubVersion.size() > MAX_SUBVERSION_LENGTH)
		return InitError(strprintf(_("Total length of network version string (%i) exceeds maximum length (%i). Reduce the number or size of uacomments."), strSubVersion.size(), MAX_SUBVERSION_LENGTH));
	...
	while(!fLoaded)
	{
		...
		do
		{
			try
			{
				...
				if(fHavePruned &&
					GetArg("-checkblocks", DEFAULT_CHECKBLOCKS) > MIN_BLOCKS_TO_KEEP)
				{
		...
		if(!fLoaded)
		{
			...
			if(!fReset)
			{
				bool fRet = uiInterface.ThreadSafeQuestion(strLoadError + ".\n\n" + _("Do you want to rebuild the block database now?"), strLoadError + ".\nPlease restart with -reindex or -reindex-chainstate to recover.", "", CClientUIInterface::MSG_ERROR | CClientUIInterface::BTN_ABORT);
