...
namespace
{
	...
	bool MarkBlockAsInFlight(NodeId nodeid, const uint256 &hash, const Consensus::Params &consensusParams, CBlockIndex *pindex = NULL, list<QueuedBlock>::iterator **pit = NULL)
	{
		...
		QueuedBlock QueuedBlockZZT;

		QueuedBlockZZT.hash = hash;
		QueuedBlockZZT.pindex = pindex;
		QueuedBlockZZT.fValidatedHeaders = (pindex != NULL);
		QueuedBlockZZT.partialBlock = std::unique_ptr<PartiallyDownloadedBlock>(pit ? new PartiallyDownloadedBlock(&mempool) : NULL);

		//list<QueuedBlock>::iterator it = state->vBlocksInFlight.insert(state->vBlocksInFlight.end(), QueuedBlockZZT);

		list<QueuedBlock>::iterator it;
...
bool AddOrphanTx(const CTransaction &tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
	...
	COrphanTx COrphanTxZZT;

	COrphanTxZZT.tx = tx;
	COrphanTxZZT.fromPeer = peer;
	COrphanTxZZT.nTimeExpire = GetTime() + ORPHAN_TX_EXPIRE_TIME;

	auto ret = mapOrphanTransactions.insert(std::pair<uint256, COrphanTx>(hash, COrphanTxZZT));
	...
	else
	{
		...
		for(size_t txinIndex = 0 ; txinIndex < tx.vin.size() ; txinIndex++)
		{
			...
			if(!viewMemPool.GetCoins(txin.prevout.hash, coins))
				return error("%s: Missing input", __FUNCTION__);
				...
				std::vector<uint256> &vHashTxnToUncache)
{
	...
	if(!GetBoolArg("-prematurewitness",false) &&
		!tx.wit.IsNull() && !witnessEnabled)
	{
...

	{
		...
		if(setConflicts.size())
		{
			...
			BOOST_FOREACH(const uint256 &hashConflicting, setConflicts)
			{
				...
				if(newFeeRate <= oldFeeRate)
					return state.DoS(0, false,
							...
							strprintf("rejecting replacement %s; new feerate %s <= old feerate %s", hash.ToString(), newFeeRate.ToString(), oldFeeRate.ToString()));
			...
			if(nModifiedFees < nConflictingFees)
				return state.DoS(0, false,
						REJECT_INSUFFICIENTFEE, "insufficient fee", false,
						strprintf("rejecting replacement %s, less fees than conflicting txs; %s < %s", hash.ToString(), FormatMoney(nModifiedFees), FormatMoney(nConflictingFees)));
			...
			if(nDeltaFees < ::minRelayTxFee.GetFee(nSize))
				return state.DoS(0, false,
						REJECT_INSUFFICIENTFEE, "insufficient fee", false,
						strprintf("rejecting replacement %s, not enough additional fees to relay; %s < %s", hash.ToString(), FormatMoney(nDeltaFees), FormatMoney(::minRelayTxFee.GetFee(nSize))));
		}
		...
		if(!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, txdata))
		{
			...
			if(tx.wit.IsNull() &&
				CheckInputs(tx, state, view, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, txdata) &&
		...
		if(!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, txdata))
			return error("%s: BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not STANDARD flags %s, %s", __FUNCTION__, hash.ToString(), FormatStateMessage(state));
...
bool ReadBlockFromDisk(CBlock &block, const CDiskBlockPos &pos, const Consensus::Params &consensusParams)
{
	...
	{
		return error("%s: Deserialize or I/O error - %s at %s", __FUNCTION__, e.what(), pos.ToString());
	}
...
void Misbehaving(NodeId pnode, int howmuch)
{
	...
	{
		LogPrintf("%s: %s (%d -> %d) BAN THRESHOLD EXCEEDED\n", __FUNCTION__, state->name, state->nMisbehavior - howmuch, state->nMisbehavior);
...
void UpdateCoins(const CTransaction &tx, CCoinsViewCache &inputs, CTxUndo &txundo, int nHeight)
{
	...
	if(!tx.IsCoinBase())
	{
		...
		BOOST_FOREACH(const CTxIn &txin, tx.vin)
		{
			...
			if(nPos >= coins->vout.size() ||
				coins->vout[nPos].IsNull())
			{
				assert(false);
			}
...
namespace Consensus
{
	bool CheckTxInputs(const CTransaction &tx, CValidationState &state, const CCoinsViewCache &inputs, int nSpendHeight)
	{
		...
		if(nValueIn < tx.GetValueOut())
			return state.DoS(100, false, REJECT_INVALID, "bad-txns-in-belowout", false, strprintf("value in (%s) < value out (%s)", FormatMoney(nValueIn), FormatMoney(tx.GetValueOut())));
			...
			for(unsigned int i = 0 ; i < tx.vin.size() ; i++)
			{
				...
				else if(!check())
				{
					if(flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS)
					{
						...
						// non-upgraded nodes.
						CScriptCheck check2(*coins, tx, i, flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheStore, &txdata);
...
namespace
{
	bool UndoWriteToDisk(const CBlockUndo &blockundo, CDiskBlockPos &pos, const uint256 &hashBlock, const CMessageHeader::MessageStartChars &messageStart)
	{
		...
		if(fileout.IsNull())
			return error("%s: OpenUndoFile failed", __FUNCTION__);
	...
	bool UndoReadFromDisk(CBlockUndo &blockundo, const CDiskBlockPos &pos, const uint256 &hashBlock)
	{
		...
		if(hashChecksum != hasher.GetHash())
			return error("%s: Checksum mismatch", __FUNCTION__);
...
static bool ApplyTxInUndo(const CTxInUndo &undo, CCoinsViewCache &view, const COutPoint &out)
{
	...
	if(undo.nHeight != 0)
	{
		...
		if(!coins->IsPruned())
			fClean = fClean && error("%s: undo data overwriting existing transaction", __FUNCTION__);
	...
	{
		if(coins->IsPruned())
			fClean = fClean && error("%s: undo data adding output to missing transaction", __FUNCTION__);
	}

	if(coins->IsAvailable(out.n))
		fClean = fClean && error("%s: undo data overwriting existing output", __FUNCTION__);
		...
		CCoinsViewCache &view, const CChainParams &chainparams, bool fJustCheck)
{
	...
	if(!CheckBlock(block, state, chainparams.GetConsensus(), !fJustCheck, !fJustCheck))
		return error("%s: Consensus::CheckBlock: %s", __FUNCTION__, FormatStateMessage(state));
	...
	if(fCheckpointsEnabled)
	{
		...
		if(pindexLastCheckpoint &&
			pindexLastCheckpoint->GetAncestor(pindex->nHeight) == pindex)
		{
	...
	for(unsigned int i = 0 ; i < block.vtx.size() ; i++)
	{
		...
		if(!tx.IsCoinBase())
		{
			...
			if(!SequenceLocks(tx, nLockTimeFlags, &prevheights, *pindex))
				return state.DoS(100, error("%s: contains a non-BIP68-final transaction", __FUNCTION__), REJECT_INVALID, "bad-txns-nonfinal");
		}
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams &chainParams)
{
	...
	LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utx)", __FUNCTION__,
...
bool static DisconnectTip(CValidationState &state, const CChainParams &chainparams, bool fBare = false)
{
	...
	if(!fBare)
	{
		...
		BOOST_FOREACH(const CTransaction &tx, block.vtx)
		{
			...
			if(tx.IsCoinBase() ||
				!AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL, true))
			{
...
static bool ActivateBestChainStep(CValidationState &state, const CChainParams &chainparams, CBlockIndex *pindexMostWork, const CBlock *pblock, bool &fInvalidFound, std::list<CTransaction> &txConflicted, std::vector<std::tuple<CTransaction, CBlockIndex *, int>> &txChanged)
{
	...
	while(fContinue &&
		nHeight != pindexMostWork->nHeight)
	{
...
bool ReceivedBlockTransactions(const CBlock &block, CValidationState &state, CBlockIndex *pindexNew, const CDiskBlockPos &pos)
{
	...
	{
		if(pindexNew->pprev &&
			pindexNew->pprev->IsValid(BLOCK_VALID_TREE))
		{
...
static bool CheckIndexAgainstCheckpoint(const CBlockIndex *pindexPrev, CValidationState &state, const CChainParams &chainparams, const uint256 &hash)
{
	...
	{
		return state.DoS(100, error("%s: forked chain older than last checkpoint (height %d)", __FUNCTION__, nHeight));
	}
...
bool ContextualCheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, const CBlockIndex *pindexPrev)
{
	...
	if(VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == THRESHOLD_ACTIVE)
	{
		...
		if(commitpos != -1)
		{
			...
			{
				return state.DoS(100, false, REJECT_INVALID, "bad-witness-nonce-size", true, strprintf("%s : invalid witness nonce size", __FUNCTION__));
			}

			CHash256().Write(hashWitness.begin(), 32).Write(&block.vtx[0].wit.vtxinwit[0].scriptWitness.stack[0][0], 32).Finalize(hashWitness.begin());

			if(memcmp(hashWitness.begin(), &block.vtx[0].vout[commitpos].scriptPubKey[6], 32))
				return state.DoS(100, false, REJECT_INVALID, "bad-witness-merkle-match", true, strprintf("%s : witness merkle commitment mismatch", __FUNCTION__));
	...
	if(!fHaveWitness)
	{
		for(size_t i = 0 ; i < block.vtx.size() ; i++)
		{
			if(!block.vtx[i].wit.IsNull())
				return state.DoS(100, false, REJECT_INVALID, "unexpected-witness", true, strprintf("%s : unexpected witness data found", __FUNCTION__));
		}
	}
	...
	if(GetBlockWeight(block) > MAX_BLOCK_WEIGHT)
		return state.DoS(100, false, REJECT_INVALID, "bad-blk-weight", false, strprintf("%s : weight limit failed", __FUNCTION__));
...
static bool AcceptBlockHeader(const CBlockHeader &block, CValidationState &state, const CChainParams &chainparams, CBlockIndex **ppindex = NULL)
{
	...
	if(hash != chainparams.GetConsensus().hashGenesisBlock)
	{
		if(miSelf != mapBlockIndex.end())
		{
			...
			if(pindex->nStatus & BLOCK_FAILED_MASK)
				return state.Invalid(error("%s: block %s is marked invalid", __FUNCTION__, hash.ToString()), 0, "duplicate");

			return true;
		}

		if(!CheckBlockHeader(block, state, chainparams.GetConsensus()))
			return error("%s: Consensus::CheckBlockHeader: %s, %s", __FUNCTION__, hash.ToString(), FormatStateMessage(state));
		...
		if(mi == mapBlockIndex.end())
			return state.DoS(10, error("%s: prev block not found", __FUNCTION__), 0, "bad-prevblk");

		pindexPrev = (*mi).second;

		if(pindexPrev->nStatus & BLOCK_FAILED_MASK)
			return state.DoS(100, error("%s: prev block invalid", __FUNCTION__), REJECT_INVALID, "bad-prevblk");
		...
		{
			return error("%s: CheckIndexAgainstCheckpoint(): %s", __FUNCTION__, state.GetRejectReason().c_str());
		}

		if(!ContextualCheckBlockHeader(block, state, chainparams.GetConsensus(), pindexPrev, GetAdjustedTime()))
			return error("%s: Consensus::ContextualCheckBlockHeader: %s, %s", __FUNCTION__, hash.ToString(), FormatStateMessage(state));
	}
...
static bool AcceptBlock(const CBlock &block, CValidationState &state, const CChainParams &chainparams, CBlockIndex **ppindex, bool fRequested, const CDiskBlockPos *dbp, bool *fNewBlock)
{
		...
		!ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindex->pprev))
	{
		...
		return error("%s: %s", __FUNCTION__, FormatStateMessage(state));
	}
...
bool ProcessNewBlock(CValidationState &state, const CChainParams &chainparams, CNode *pfrom, const CBlock *pblock, bool fForceProcessing, const CDiskBlockPos *dbp, CConnman *connman)
{
	...
	if(!ActivateBestChain(state, chainparams, pblock, connman))
		return error("%s: ActivateBestChain failed", __FUNCTION__);
...
bool TestBlockValidity(CValidationState &state, const CChainParams &chainparams, const CBlock &block, CBlockIndex *pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
	...
	if(fCheckpointsEnabled &&
		!CheckIndexAgainstCheckpoint(pindexPrev, state, chainparams, block.GetHash()))
	{
		return error("%s: CheckIndexAgainstCheckpoint(): %s", __FUNCTION__, state.GetRejectReason().c_str());
	}
	...
	if(!ContextualCheckBlockHeader(block, state, chainparams.GetConsensus(), pindexPrev, GetAdjustedTime()))
		return error("%s: Consensus::ContextualCheckBlockHeader: %s", __FUNCTION__, FormatStateMessage(state));

	if(!CheckBlock(block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
		return error("%s: Consensus::CheckBlock: %s", __FUNCTION__, FormatStateMessage(state));

	if(!ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindexPrev))
		return error("%s: Consensus::ContextualCheckBlock: %s", __FUNCTION__, FormatStateMessage(state));
...
CBlockIndex *InsertBlockIndex(uint256 hash)
{
	...
	if(!pindexNew)
		throw runtime_error(std::string(__FUNCTION__) + ": new CBlockIndex failed");
...
bool static LoadBlockIndexDB()
{
	...
	LogPrintf("%s: last block file = %i\n", __FUNCTION__, nLastBlockFile);

	for(int nFile = 0 ; nFile <= nLastBlockFile ; nFile++)
		pblocktree->ReadBlockFileInfo(nFile, vinfoBlockFile[nFile]);

	LogPrintf("%s: last block file info: %s\n", __FUNCTION__, vinfoBlockFile[nLastBlockFile].ToString());
	...
	LogPrintf("%s: transaction index %s\n", __FUNCTION__, fTxIndex ? "enabled" : "disabled");
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
	...
	for(CBlockIndex *pindex = chainActive.Tip() ; pindex && pindex->pprev ; pindex = pindex->pprev)
	{
		...
		{
			return error("%s: *** found bad block at %d, hash=%s (%s)\n", __FUNCTION__, pindex->nHeight, pindex->GetBlockHash().ToString(), FormatStateMessage(state));
		}
	...
	try
	{
		...
		while(!blkdat.eof())
		{
			...
			try
			{
				...
				if(nSize < 80 ||
					nSize > MAX_BLOCK_SERIALIZED_SIZE)
				{
					continue;
				}
			...
			try
			{
				...
				{
					LogPrint("reindex", "%s: Out of order block %s, parent %s not known\n", __FUNCTION__, hash.ToString(), block.hashPrevBlock.ToString());
				...
				while(!queue.empty())
				{
					...
					while(range.first != range.second)
					{
						...
						{
							LogPrint("reindex", "%s: Processing out of order child %s of %s\n", __FUNCTION__, block.GetHash().ToString(), head.ToString());
							LOCK(cs_main);
...

		{
				...
				inv.type == MSG_WITNESS_BLOCK)
			{
				...
				if(send &&
					...
					(((pindexBestHeader != NULL) && (pindexBestHeader->GetBlockTime() - mi->second->GetBlockTime() > nOneWeek)) || inv.type == MSG_FILTERED_BLOCK) &&
					!pfrom->fWhitelisted)
				{
...
uint32_t GetFetchFlags(CNode *pfrom, CBlockIndex *pprev, const Consensus::Params &chainparams)
{
	...
	if(IsWitnessEnabled(pprev, chainparams) &&
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived, const CChainParams &chainparams, CConnman &connman)
{
	...
	if(!(pfrom->GetLocalServices() & NODE_BLOOM) &&
		(strCommand == NetMsgType::FILTERLOAD || strCommand == NetMsgType::FILTERADD || strCommand == NetMsgType::FILTERCLEAR))
	{
	...
	if(strCommand == NetMsgType::VERSION)
	{
		...
		if(pfrom->fInbound &&
			!connman.CheckIncomingNonce(nNonce))
		{
	...
	else if(strCommand == NetMsgType::TX)
	{
		...
		if(state.IsInvalid(nDoS))
		{
			...
			if(state.GetRejectCode() < REJECT_INTERNAL) // Never send AcceptToMemoryPool's internal codes over P2P
				pfrom->PushMessage(NetMsgType::REJECT, strCommand, (unsigned char)state.GetRejectCode(), state.GetRejectReason().substr(0, MAX_REJECT_MESSAGE_LENGTH), inv.hash);
		...
		!fImporting && !fReindex) // Ignore blocks received while importing
	{
		...
		else
		{
			...
			{
				assert(state.GetRejectCode() < REJECT_INTERNAL); // Blocks are never rejected with internal reject codes
				pfrom->PushMessage(NetMsgType::REJECT, strCommand, (unsigned char)state.GetRejectCode(), state.GetRejectReason().substr(0, MAX_REJECT_MESSAGE_LENGTH), block.GetHash());
		...
		it != pfrom->vRecvMsg.end())
	{
		...
		}catch(const std::ios_base::failure &e)
		{
			...
			if(strstr(e.what(), "end of data"))
			{
				...
				LogPrintf("%s(%s, %u bytes): Exception '%s' caught, normally caused by a message being shorter than its stated length\n", __FUNCTION__, SanitizeString(strCommand), nMessageSize, e.what());
			}
		...
		if(!fRet)
			LogPrintf("%s(%s, %u bytes) FAILED peer=%d\n", __FUNCTION__, SanitizeString(strCommand), nMessageSize, pfrom->id);
