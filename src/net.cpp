...

		{
			...
			BOOST_FOREACH(CNode *pnode, vNodes)
			{
...

				{
					...
					if(lockRecv &&
						(pnode->vRecvMsg.empty() || !pnode->vRecvMsg.front().complete() || pnode->GetTotalRecvSize() <= GetReceiveFloodSize()))
					{
		...
		if(LookupHost(pszHostName, vaddr, 0, true))
		{
			BOOST_FOREACH(const CNetAddr &addr, vaddr)
			{
				if(AddLocal(addr, LOCAL_IF))
					LogPrintf("%s: %s - %s\n", __FUNCTION__, pszHostName, addr.ToString());
			}
		}
	}
#else
