...
void BitcoinCore::initialize()
{
	try
	{
		qDebug() << __FUNCTION__ << ": Running AppInit2 in thread";
...
void BitcoinCore::shutdown()
{
	try
	{
		qDebug() << __FUNCTION__ << ": Running Shutdown in thread";
		Interrupt(threadGroup);
		threadGroup.join_all();
		Shutdown();
		qDebug() << __FUNCTION__ << ": Shutdown finished";
...
BitcoinApplication::~BitcoinApplication()
{
	if(coreThread)
	{
		qDebug() << __FUNCTION__ << ": Stopping thread";
		Q_EMIT stopThread();
		coreThread->wait();
		qDebug() << __FUNCTION__ << ": Stopped thread";
	}
...
{
	qDebug() << __FUNCTION__ << ": Requesting initialize";
	startThread();
...
{
	qDebug() << __FUNCTION__ << ": Requesting shutdown";
	startThread();
	window->hide();
...
{
	qDebug() << __FUNCTION__ << ": Initialization result: " << retval;
...
{
	qDebug() << __FUNCTION__ << ": Shutdown result: " << retval;
...
int main(int argc, char *argv[])
{
	...
	{
		QMessageBox::critical(0, QObject::tr(PACKAGE_NAME), QObject::tr("Error: Specified data directory \"%1\" does not exist.").arg(QString::fromStdString(mapArgs["-datadir"])));
