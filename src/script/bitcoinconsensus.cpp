...
public:
    ...
    TxInputStream& read(char* pch, size_t nSize)
    {
        if (nSize > m_remaining)
			throw std::ios_base::failure(std::string(__FUNCTION__) + ": end of data");

        if (pch == NULL)
			throw std::ios_base::failure(std::string(__FUNCTION__) + ": bad destination buffer");

        if (m_data == NULL)
			throw std::ios_base::failure(std::string(__FUNCTION__) + ": bad source buffer");
