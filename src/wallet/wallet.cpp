...
CPubKey CWallet::GenerateNewKey()
{
	...
	if(IsHDEnabled())
	{
		...
		if(!GetKey(hdChain.masterKeyID, key))
			throw std::runtime_error(std::string(__FUNCTION__) + ": Master key not found");
		...
		if(!CWalletDB(strWalletFile).WriteHDChain(hdChain))
			throw std::runtime_error(std::string(__FUNCTION__) + ": Writing HD chain model failed");
	} else
	...
	if(!AddKeyPubKey(secret, pubkey))
		throw std::runtime_error(std::string(__FUNCTION__) + ": AddKey failed");
...
bool CWallet::LoadKeyMetadata(const CPubKey &pubkey, const CKeyMetadata &meta)
{
	...
	if(meta.nCreateTime &&
		(!nTimeFirstKey || meta.nCreateTime < nTimeFirstKey))
	{
...
CAmount CWallet::GetCredit(const CTxOut &txout, const isminefilter &filter) const
{
	if(!MoneyRange(txout.nValue))
		throw std::runtime_error(std::string(__FUNCTION__) + ": value out of range");
...
CPubKey CWallet::GenerateNewHDMasterKey()
{
...

	{
		...
		if(!AddKeyPubKey(key, pubkey))
			throw std::runtime_error(std::string(__FUNCTION__) + ": AddKeyPubKey failed");
	}
...
bool CWallet::SetHDChain(const CHDChain &chain, bool memonly)
{
	...
	{
		throw runtime_error(std::string(__FUNCTION__) + ": writing chain failed");
	}
...
{
	{
		...
		while(setKeyPool.size() < (nTargetSize + 1))
		{
			...
			if(!walletdb.WritePool(nEnd, CKeyPool(GenerateNewKey())))
				throw runtime_error(std::string(__FUNCTION__) + ": writing generated key failed");
...
void CWallet::ReserveKeyFromKeyPool(int64_t &nIndex, CKeyPool &keypool)
{
...

	{
		...
		if(!walletdb.ReadPool(nIndex, keypool))
			throw runtime_error(std::string(__FUNCTION__) + ": read failed");

		if(!HaveKey(keypool.vchPubKey.GetID()))
			throw runtime_error(std::string(__FUNCTION__) + ": unknown key in key pool");
...
int64_t CWallet::GetOldestKeyPoolTime()
{
	...
	if(!walletdb.ReadPool(nIndex, keypool))
		throw runtime_error(std::string(__FUNCTION__) + ": read oldest key in keypool failed");
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t> &mapKeyBirth) const
{
	...
	for(std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin() ; it != mapWallet.end() ; it++)
	{
		...
		if(blit != mapBlockIndex.end() &&
			chainActive.Contains(blit->second))
		{
	...
	if(fFirstRun)
	{
			...
			!walletInstance->IsHDEnabled())
		{
			...
			if(!walletInstance->SetHDMasterKey(masterPubKey))
				throw std::runtime_error(std::string(__FUNCTION__) + ": Storing master key failed");
		}
...
bool CWallet::ParameterInteraction()
{
	...
	if(mapArgs.count("-paytxfee"))
	{
		...
		if(payTxFee < ::minRelayTxFee)
			return InitError(strprintf(_("Invalid amount for -paytxfee=<amount>: '%s' (must be at least %s)"), mapArgs["-paytxfee"], ::minRelayTxFee.ToString()));
	}
	...
	if(mapArgs.count("-maxtxfee"))
	{
		...
		if(CFeeRate(maxTxFee, 1000) < ::minRelayTxFee)
			return InitError(strprintf(_("Invalid amount for -maxtxfee=<amount>: '%s' (must be at least the minrelay fee of %s to prevent stuck transactions)"), mapArgs["-maxtxfee"], ::minRelayTxFee.ToString()));
	}
