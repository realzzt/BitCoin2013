...
{
	if(Read(std::string("bestblock"), locator) &&
		!locator.vHave.empty())
	{
		return true;
	}
...
void CWalletDB::ListAccountCreditDebit(const string &strAccount, list<CAccountingEntry> &entries)
{
	...
	if(!pcursor)
		throw runtime_error(std::string(__FUNCTION__) + ": cannot create DB cursor");
	...
	while(true)
	{
		...
		{
			pcursor->close();
			throw runtime_error(std::string(__FUNCTION__) + ": error scanning DB");
		}
